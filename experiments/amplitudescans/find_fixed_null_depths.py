...
PARENT_DIR = str(glint_paths.DATA_ROOT / 'alignment_scans' / 'amplitudescans' / '2025' / '07-02' / 'scan1')
BASELINE = ["11", "31"]
...
for subdir in sorted(os.listdir(PARENT_DIR)):
    ...
    if SAVE_NULLSCAN_PLOT:
        plt.figure()
        plt.plot(opd, normalised_flux*np.max(summed_flux), 'o', color='C0', label='Data')
        if 'fit_curve' in locals():
            plt.plot(opd_dense, fit_curve*np.max(summed_flux), '-', color='k', label='Fit')
            plt.axvline(min_opd, color='red', linestyle='--', label=f"Min = {min_flux*np.max(summed_flux):.3f} at {min_opd:.3f} μm")
        if dark_flux_norm is not None:
            plt.plot(dark_opd_crop, dark_flux_norm*np.max(summed_flux), color='black', linestyle='--', label='Dark')
