...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        print('Please provide: python find_psf.fits FOLDER FILENAME')
        sys.exit(1)
    folder = sys.argv[1] 
    filename = sys.argv[2]
    darkfilename = 'dark.fits'

    hdul = fits.open(path + folder+ filename)
    img = hdul[0].data
    img = img.astype(float)

    hdul = fits.open(path + folder+ darkfilename)
    dark = hdul[0].data
    ...
    plt.title('Goal (21/10/25) PSF centre: ({:.1f}, {:.1f})'.format(goalx,goaly))
