"""
Raster-scans the mount over two axes (default: pitch and yaw), recording
APAPANE spectral photometry at each grid point, then saves the results as a
FITS cube and hands off to read_pitchyawscan.py for fitting.
...
PARAM_FILE = str(glint_paths.CODE_ROOT / 'benchalignment' / 'chipmountalignment' / 'pitchyawscans' / 'scanparameters.json')
DARK_FILEPATH = str(glint_paths.DATA_ROOT / 'benchalignment' / 'spectraldark.fits')
ANALYSIS_SCRIPT = str(glint_paths.CODE_ROOT / 'benchalignment' / 'chipmountalignment' / 'pitchyawscans' / 'read_pitchyawscan.py')
...
def open_devices():
    ...
    -------
    apapane : SHM
        Shared-memory handle for the APAPANE (spectrograph) camera.
...
def getbox(peak: int, boundingvals: list, box_halfwidth: int) -> list:
    ...
    Note, the CRED1 detector is now in use, where spectra run horizontally (so `peak` indexes rows and `boundingvals` indexes columns). 
...
def getdata(apapane: SHM, box: list, dark: np.ndarray, nframes: int = 1) -> np.ndarray:
    """
    Eventually will be outdated and instead spectral extraction code will be used. 
...
def savedata(path: str, photometry_scans, mount_positions: np.ndarray,
    ...
    select_axes : list
        The two axis names scanned, e.g. ['pitch', 'yaw'].
    iteration : int
...
def rasterscan(apapane: SHM, mount: Mount, dark: np.ndarray, select_axes: list,
    ...
    select_axes : list
        The two axis names to scan, e.g. ['pitch', 'yaw'].
...
if __name__ == "__main__":
    ...
    base_dir = str(glint_paths.data_dir('benchalignment', 'chipmountalignment', 'pitchyawscans', 'scanoutput', year, date))
    ...
    try:
        subprocess.run(
            ["python", ANALYSIS_SCRIPT, "--update-param-file", "--method", "elliptical_gaussian", "--show"],
            check=True,
        )
        print("Pitch-yaw analysis complete and parameters updated.")
    except subprocess.CalledProcessError as e:
        print("Error running pitch-yaw analysis script:")
        print(e)
