...
current year/date/iteration from scanparameters.json, opens the matching FITS
files, turns each photometry cube into a 2D pitch/yaw heatmap, estimates the
best common centre, saves PNG/JSON outputs, and can optionally write the best
...
def get_scan_dir(params: dict, base_root: str | Path, iteration: int | None = None) -> Path:
    """Build the scan directory from year/date/iteration in scanparameters.json."""
    scan_iteration = int(params["iteration"] if iteration is None else iteration)
    return Path(base_root) / str(params["year"]) / str(params["date"]) / f"scan{scan_iteration}"
...
def gaussian_peak(image: np.ndarray, threshold_fraction: float = 0.6) -> Tuple[float, float]:
    """
    Fit a 2D Gaussian to the brightest part of a heatmap.

    Returns (x, y) in pixel coordinates. If scipy is unavailable or the fit fails,
    returns (nan, nan), and the caller can fall back to centroid/max.
    """
    if not SCIPY_AVAILABLE:
        return np.nan, np.nan

    fit_image = np.asarray(image, dtype=float).copy()
    fit_image = fit_image - np.nanmin(fit_image)

    max_value = np.nanmax(fit_image)
    if not np.isfinite(max_value) or max_value <= 0:
        return np.nan, np.nan

    fit_image[fit_image < threshold_fraction * max_value] = 0
    ...
    initial_guess = (
        ...
        max(y_size / 4, 1),
        0.0,
        float(np.nanmedian(fit_image)),
    )

    try:
        popt, _ = curve_fit(two_d_gaussian, (x, y), fit_image.ravel(), p0=initial_guess, maxfev=10000)
        _, xo, yo, *_ = popt
...
def find_optimum(
    ...
    method options:
        ...
        gaussian  : 2D Gaussian fit, falling back to centroid, then max
    """
    method = method.lower()
    ...
    if method == "centroid":
        ...
        x_pix, y_pix = gaussian_peak(heatmap, threshold_fraction=threshold_fraction)
        used = "gaussian"
        if not (np.isfinite(x_pix) and np.isfinite(y_pix)):
            x_pix, y_pix = centre_of_mass(heatmap)
            used = "centroid_fallback"
    else:
        raise ValueError("method must be one of: max, centroid, gaussian")
...
def main() -> None:
    ...
    parser.add_argument("--method", choices=["max", "centroid", "gaussian"], default="gaussian", help="Optimum-finding method")
    parser.add_argument("--threshold-fraction", type=float, default=0.6, help="Threshold fraction for Gaussian fit")
    ...
    if args.update_param_file:
        # These keys are deliberately simple. Your pitchyawscan.py does not yet use
        # them automatically, but saving them here makes the result available to the
        # scan script/GUI later.
        params["pitch"] = results["common_centre"]["pitch"]
        params["yaw"] = results["common_centre"]["yaw"]
        params["fitted_pitchyaw"] = results["common_centre"]
