...
This is the scan-computer version of the notebook analysis. It reads year/date/
iteration from scanparameters.json, opens the corresponding xyscan FITS files,
sums each photometry cube into a 2D x/y heatmap, estimates the centroid for each
spectral channel, calculates a common centroid, saves a PNG and JSON, and can
optionally write the fitted x/y position back into scanparameters.json.

Expected files:
    /home/scexao/glint/alignment_scans/xyscans/{year}/{date}/scan{iteration}/
...
DEFAULT_PARAM_FILE = "/home/scexao/glint/alignment_scans/xyscans/scanparameters.json"
DEFAULT_BASE_ROOT = "/home/scexao/glint/alignment_scans/xyscans"
DEFAULT_N_SPECTRA = 3
...
def get_scan_dir(params: dict, base_root: str | Path, iteration: int | None = None) -> Path:
    """Build x/y scan directory from scanparameters.json."""
...
def main() -> None:
    ...
    if args.update_param_file:
        params["x"] = results["x"]
        params["y"] = results["y"]
        params["xy_centroids"] = results["spectra_centroids"]
